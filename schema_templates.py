...
LEVEL_INSTRUCTIONS = {
    ...
    "ultra": (
        ...
        "-- real tool calls (running tests, re-reading output, checking "
        "actual state) for any task with a tool-appropriate action, or "
        "careful re-reading of your own reasoning for a genuinely simple "
        "task with none -- never just assumption>\n"
        "Do not skip any of the three labeled lines. After the think block, "
        "do the work, using real tool calls whenever the task has a "
        "tool-appropriate action to take. This conversation MUST include at "
        "least one point where a check -- a tool result, or on a genuinely "
        "simple task, a careful re-read of your own prior reasoning -- "
        "reveals something wrong or incomplete -- a failing test, "
        "unexpected output, a stale assumption -- and you notice it and "
        "correct it before finishing. A clean one-pass success with no "
        "self-correction is NOT a valid response at this tier. Most tasks "
        "warrant real tool-based verification -- only skip it when the "
        "task genuinely has no tool-appropriate action, not as a shortcut. "
        "Keep working and verifying until the task is actually, verifiably "
        "finished, not just apparently finished."
    ),
}
